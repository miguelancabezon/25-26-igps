...
```mermaid
flowchart TD
    A[Portland] --> B[Red Light District]
    A --> C[Trenton]
    A --> D[Saint Mark's]
    B --> E[Mafia Leone]
    C --> F[Diablos]
    D --> G[Triadas]
    E --> H[Misiones iniciales]
    F --> H
    G --> H
```
...
sequenceDiagram
    ...
    participant Joey
    participant Toni
    participant Salvatore

    Claude->>Luigi: Recibe primeras misiones
    Luigi->>Claude: Introduce a Joey
    Claude->>Joey: Realiza trabajos mecánicos
    Joey->>Claude: Conecta con Toni
    Toni->>Claude: Misiones para la Mafia
    Claude->>Salvatore: Escala en la organización
```

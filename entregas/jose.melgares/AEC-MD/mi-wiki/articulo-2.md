...
```mermaid
flowchart TD
    A[Staunton Island] --> B[Downtown]
    A --> C[Liberty Campus]
    A --> D[Newport]
    B --> E[Yakuza]
    C --> F[Yardies]
    D --> G[Colombianos]
    E --> H[Conflictos entre bandas]
    F --> H
    G --> H
```
...
sequenceDiagram
    ...
    participant Asuka
    participant Kenji
    participant Donald Love
    participant Catalina

    Claude->>Asuka: Trabaja para los Yakuza
    Asuka->>Kenji: Misiones de honor
    Claude->>Donald Love: Encargos políticos
    Donald Love->>Claude: Manipula conflictos
    Claude->>Catalina: Reencuentro y traición
```

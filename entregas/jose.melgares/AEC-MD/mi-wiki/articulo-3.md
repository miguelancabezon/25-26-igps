...
```mermaid
flowchart TD
    A[Shoreside Vale] --> B[Cedar Grove]
    A --> C[Wichita Gardens]
    A --> D[Pike Creek]
    B --> E[Cartel Colombiano]
    C --> F[Residentes]
    D --> G[Industria ilegal]
    E --> H[Enfrentamientos finales]
    F --> H
    G --> H
```
...
sequenceDiagram
    ...
    Claude->>Miguel: Obtiene información
    Miguel->>Claude: Revela planes del Cartel
    Claude->>Catalina: Confrontación final
    Catalina->>Claude: Secuestra a Maria
    Claude->>Cartel: Batalla decisiva
    Claude->>Maria: Rescate y desenlace
